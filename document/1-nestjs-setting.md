이전에는 [PUREMVC](https://puremvc.org/)를 사용하고 있었다. 심지어 MVC 프레임워크이며, 여러가지 개발 언어를 지원하고 있고, 여러가지 패턴(pattern)을 사용 할 수 있었다.
내가 nestjs로 갈아탄 이유는 여러가지가 있지만 그 중 하나가 puremvc 가 더 이상 업데이트를 지원하지 않고 있기도 하고 자바보다 나은 환경을 제공하는 느낌을 받았습니다.
